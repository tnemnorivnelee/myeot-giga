...
**All feature/bug work happens on a branch. `main` only holds finished work.**

- Branch name: `feature/<issue-number>-<name>` (e.g. `feature/12-widget-percent-mode`); bug fixes use `fix/<issue-number>-<description>`
- Docs/config work with no issue number: name by intent, e.g. `docs/<description>`, `chore/<description>`
- Create a new branch from `main` before starting work
- Open a PR when done; self-review the diff before merging
- Not enforced (no branch protection) — kept as a habit-based convention
